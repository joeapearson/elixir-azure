defmodule Azure.Storage.BlobPolicy do
  @moduledoc """
  BlobPolicy
  """

  import SweetXml
  import Azure.Storage.DateTimeUtils
  import Azure.Storage.Utilities, only: [set_to_string: 2, string_to_set: 2]
  require EEx
  ...
  <SignedIdentifiers>
    ...
    <SignedIdentifier>
      <Id><%= policy.id %></Id>
      <AccessPolicy>
        <Start><%= policy.start |> Azure.Storage.DateTimeUtils.to_string_iso8601() %></Start>
        <Expiry><%= policy.expiry |> Azure.Storage.DateTimeUtils.to_string_iso8601() %></Expiry>
        <Permission><%= policy.permission |> Azure.Storage.BlobPolicy.permission_serialize() %></Permission>
      </AccessPolicy>Date
    </SignedIdentifier>
    <% end %>
  </SignedIdentifiers>
